...
type DeleteAttendanceSheetProps = {
...
const DeleteAttendanceSheet = ({navigation}: DeleteAttendanceSheetProps) => {
  ...
  const handleSubmit = async (values: typeof defaultValues) => {
    ...
    if (status === HttpStatusCode.OK && data) {
      navigation.navigate('select_attendance', {
        data: data.data,
        type: 'delete',
        form_data: values,
      });
...
export default DeleteAttendanceSheet;
